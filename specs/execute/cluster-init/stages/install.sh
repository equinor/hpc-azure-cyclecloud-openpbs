...
if [[ -f "$NODE_CREATED_GUARD" ]]; then
    ...
    #NODE_ID=$(jetpack config cyclecloud.node.id) || fail
    #if ! echo "$NODE_ATTRS" | bool grep -qi "$NODE_ID"; then
    NNAMES=$( /opt/pbs/bin/pbsnodes "$EXECUTE_HOSTNAME" |& grep -ci "$EXECUTE_HOSTNAME" )
    if [ "$NNAMES" -lt 2 ]; then
